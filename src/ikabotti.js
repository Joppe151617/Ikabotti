...
async function dailyMissionsReminder() {
	...
	while (true) {
		...
		var min10 = 600;
		var min = 60;
		var deadline = 1600034399; // 7.5.18 00:59:59 gmt+2 Talviaika: 1525647599 Kesäaika: 1525643999. 14.9.20 00:59:59 1600034399
...
async function pirateReminder() {
	...
	while (true) {
		...
		var min5 = 300;
		var min = 60;
		var deadline = 1547337600; // 13.1.19 3:00:00 gmt+2 Talviaika: 1547341200 Kesäaika: 1547337600
		...
		if(timeLeft < min5) {
			let ikarkoodaritChannel = client.channels.get("451458886470336523");
			ikarkoodaritChannel.send("Aika laittaa 16h merirosvot matkaan!!");
			ekaViesti = true;
			await sleep(min5);
		}
		await sleep(min);
